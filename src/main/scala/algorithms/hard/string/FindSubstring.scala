...
object FindSubstring {
  ...
  /** RunTime Info: 2264 ms, 58.2 MB
    *
    * @param s
    *   source string to search
    * @param words
    *   list of target string
    * @return
    *   all the index where we can get the concatenation of all the string in `words`
    */
  def findSubstring(s: String, words: Array[String]): List[Int] = {
    ...
    def travel(wc: Map[String, Int], str: String): Boolean =
      if (wc.isEmpty || empty(wc)) {
        ...
        wc.keys
          .filter(key => str.startsWith(key) && wc(key) > 0)
          .map { key =>
            var wcc = wc
            wcc += key -> (wcc(key) - 1)
