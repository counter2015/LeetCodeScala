...
object TextJustification {
  ...
  /** RunTime Info:
    * 244 ms, 46.2 MB
    *
    * @param words    words string in array
    * @param maxWidth the max width of each line
    * @return fixed lines with max width
    */
  def fullJustify(words: Array[String], maxWidth: Int): List[String] = {
    ...
    if (l <= maxWidth) {
      ...
      if (headWords.length > 1) {
        ...
        val spacesArray = List.fill(moreSpaceCount)(spaceCount + 1) ++ List.fill(headWords.length - 1 - moreSpaceCount)(spaceCount)
