...
object SellStock3 {
  ...
  /** RunTime Info: 2560 ms, 713.9 MB
    *
    * @param prices
    *   the prices of days
    * @return
    *   the max profit with at most two transaction
    */
  ...
  def maxProfitBetter(prices: Array[Int]): Int =
    if (prices.isEmpty) 0
      ...
      val leftProfits = prices
        .foldLeft((List.empty[Int], prices.head)) { case ((acc, min), price) =>
      ...
      val rightProfits = prices
        .foldRight((List.empty[Int], prices.last)) { case (price, (acc, max)) =>
          val lastProfit = acc.headOption.getOrElse(0)
